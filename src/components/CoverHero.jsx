    ...
    <section className="pb-10 border-b border-gray-200">
      <div className="grid lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-4">
          <a
            href={`/${coverArticle.slug}`}
            onClick={(e) => {
              if (!e.ctrlKey && !e.metaKey) {
                e.preventDefault();
                onSelectArticle(coverArticle);
              }
            }}
          ...
          <div className="text-[10px] font-bold text-black tracking-wider uppercase pt-1 border-t border-gray-200 flex items-center gap-2">
            <a
              href={`/author/${coverArticle.authorId || 'marcus-cole'}`}
              onClick={(e) => {
                if (!e.ctrlKey && !e.metaKey) {
                  e.preventDefault();
                  if (onSelectAuthor) onSelectAuthor(coverArticle.authorId || 'marcus-cole');
                }
              }}
          ...
          {stackedArticles.slice(0, 2).map((article, idx, arr) => (
            ...
            >
              <a
                ...
                className="group cursor-pointer block space-y-2"
              >
              ...
              <div className="text-[10px] font-bold text-black tracking-wider uppercase pt-1 flex items-center gap-2">
                <a
                  href={`/author/${article.authorId || 'eleanor-vance'}`}
                  onClick={(e) => {
                    if (!e.ctrlKey && !e.metaKey) {
                      e.preventDefault();
